...
get_html = soup.find_all('div', class_='cli-children')
...
for html in get_html:
    ...
    movie_name = movie_name[movie_name.index('.') + 1 : ]
    movie_dic['Movie Name'] = movie_name if movie_name else 'unknown movie name'

    # Release Date
    rel_date = html.find('span', class_='cli-title-metadata-item')
    movie_dic['Release Date'] = rel_date.text.strip() if rel_date else 'unknown release date'

    # Duration
    duration = html.find_all('span', class_='cli-title-metadata-item')[1]
